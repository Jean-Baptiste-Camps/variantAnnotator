...
class FewShotVariantDataset(Dataset):
    def __init__(self, df):
        ...
        self.system_and_examples = (
            ...
            "1. graph: variation graphémique, sans impact sur la morphologie ou la syntaxe (ex. païs (AB)|pays (FH)).\n"
            "2. flex: variation flexionnelle, telle qu'une substitution entre désinences de valeur équivalente (ex. savomes (CB)|savon (H)|savons (P)).\n"
            "3. morsynt: variation morphosyntaxique, telle que singulier vs. pluriel, masculin vs. féminin, présent vs. futur (ex. les despose (E)|le deposera (G)).\n"
            "4. semlex:minor:gramm: écart sémantico-lexical faible s’expliquant par une variante de grammème ou mot-outil (ex. cele (Ao)|l’ (Ez)).\n"
            "5. semlex:minor:constrmorph: écart sémantico-lexical faible s’expliquant par la morphologie constructionnelle, telle que l'ajout ou le changement d'un préfixe (mervoillent (Ao)|esmerveillent (Ez)).\n"
            "6. semlex:minor:syn: écart sémantico-lexical faible s’expliquant par une relation de synonymie (ex. trichier (Ao)|decevoir (Ez)).\n"
            "7. semlex:minor:semsim: écart sémantico-lexical faible s’expliquant par proximité sémantique d'un autre type, telles que hypéronimie, co-hyponimie, etc. (sergant (B)|archir (M)).\n"
            "8. semlex:major: écart sémantico-lexical fort (ex. acorde (Ao)| discorde (Ez)).\n"            
